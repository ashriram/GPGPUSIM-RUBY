...
//                 var->getType()->existPair("filter") || var->existPair("trigger_queue")
              ...
              // Set description
              if (var->getType()->isBuffer() && !var->existPair("infinite")) {
                sstr << "  m_" << var->cIdent() << "_vec[i]->setDescription(";
                sstr << "\"[Chip \" + int_to_string(getID()) + \" \" + int_to_string(i) + \", "
                     << var->getMachine()->getIdent() << ", " << var->getIdent() << "]\"";
                sstr << ");\n";
                sstr << "\n";
              }
...
//                 var->getType()->existPair("filter") || var->existPair("trigger_queue")
    ...
    sstr << "void Chip::printBufferStats(ostream& out)\n";
    sstr << "{\n";
    sstr << "  out << endl;\n";
    sstr << "  out << \"Chip Buffer Stats\" << endl;\n";
    sstr << "  out << \"----------\" << endl << endl;\n";

    // Look at all the protocol buffers
    for(int i=0; i<size; i++) {
       Var* var = dynamic_cast<Var*>(m_sym_vec[i]);
       if (var != NULL && var->existPair("chip_object") && !var->existPair("no_chip_object")) {
          ...
          if (!var->existPair("network")) {
             ...
             sstr << "  for (int i = 0; i < RubyConfig::numberOf" << var->getMachine()->getIdent()
                     << "PerChip(m_id); i++)  {" << endl;
             sstr << "    m_" << var->cIdent() << "_vec[i]->printStats(out);\n";
              ...
              if (!var->getType()->existPair("non_obj") && (!var->getType()->isEnumeration())) {
                if (var->existPair("no_vector")) {
                  ...
                  sstr << "  out << \"\\n" << var->cIdent() << " numberPerChip: \" << RubyConfig::numberOf" << var->getMachine()->getIdent()
                       << "PerChip() << endl;\n";
                  sstr << "  if(m_" << var->cIdent() << "_vec.size()>0)\n";
...
//                   sstr << "  }\n";
    ...
    sstr << "void Chip::flushAllL1DCaches(ostream& out)" << endl;
    sstr << "{" << endl;
    sstr << "  for (int i = 0; i < RubyConfig::numberOfL1CachePerChip(m_id); i++)  {" << endl;
    sstr << "    assert(m_L1Cache_L1DcacheMemory_vec.size() > i);" << endl;
    sstr << "    m_L1Cache_L1DcacheMemory_vec[i]->flushCache(out);" << endl;
    sstr << "  }" << endl;
    sstr << "}" << endl;
