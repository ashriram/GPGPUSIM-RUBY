...
{
  g_system_ptr->getProfiler()->profileBandwidth(type, size);
}

void profileBandwidthBytes(string type, int bytes, bool round_up) {
   g_system_ptr->getProfiler()->profileBandwidthBytes(type, bytes, round_up);
}

void profileLifetime(Time lifetime, IntSet SharerPCs, int addr, string event) {
   g_system_ptr->getProfiler()->profileLifetime(lifetime, SharerPCs, addr, event);
}
...
void profileCLStallEvict(Time stall_cycles){
    g_system_ptr->getProfiler()->profileCLStallEvict(stall_cycles);
}

void profileCLStallFence(Time stall_cycles){
    g_system_ptr->getProfiler()->profileCLStallFence(stall_cycles);
}

void profilePrivateL1Eviction() {
    g_system_ptr->getProfiler()->profilePrivateL1Eviction();
}
