...
public:
  ...
  virtual void flushAllL1DCaches(ostream& out) = 0;

  bool usingGpusimDram() {
      return (m_Directory_GpusimDramBuffer_vec.size() > 0) or (m_L2Cache_GpusimDramBuffer_vec.size() > 0);
  }

  GpusimDramInterface* getDramInterface(int index) {
     if(m_L2Cache_GpusimDramBuffer_vec.size() > 0) {
         assert(m_L2Cache_GpusimDramBuffer_vec.size() > index);
         return m_L2Cache_GpusimDramBuffer_vec[index];
     } else {
         assert(m_Directory_GpusimDramBuffer_vec.size() > index);
         return m_Directory_GpusimDramBuffer_vec[index];
     }
  };
  ...
  virtual void printBufferStats(ostream& out) = 0;
  ...
  Vector < MessageBuffer* > m_L1Cache_idealInvQueue_vec;        // queue for ideal invalidations

  // TSO storebuffer
  Vector < StoreBuffer* > m_L1Cache_storeBuffer_vec;

  // Interface with GPGPU-Sim's DRAM
  Vector < GpusimDramInterface* >m_Directory_GpusimDramBuffer_vec;
