...
extern inline int RealAddressToInt(Address addr)
{
...
extern inline Address intToAddress(int addr)
{
  return (Address) addr;
}
...
extern inline MessageSizeType IntToMessageSizeType(int size)
{
    return int_to_MessageSizeType(size);
}
...
extern inline Time getTimePlusTime(Time addend1, Time addend2)
{
  return (Time) addend1 + addend2;
}
...
// Returns minimum of the two times
extern inline Time getTimeMin(Time t1, Time t2)
{
    return (t1<t2?t1:t2);
}

// Returns maximum of the two times
extern inline Time getTimeMax(Time t1, Time t2)
{
    return (t1>t2?t1:t2);
}

extern inline int getIntMax(int i1, int i2)
{
    return (i1>i2?i1:i2);
}
...
extern inline bool get_GMEM_SKIP_L1D() {
   return g_GMEM_SKIP_L1D;
}

extern inline int get_DATA_BLOCK_BYTES() {
   return g_DATA_BLOCK_BYTES;
}

// And two byte masks
extern inline ruby_mem_access_byte_mask_t and_byte_masks(ruby_mem_access_byte_mask_t mask1, ruby_mem_access_byte_mask_t mask2) {
   return mask1 & mask2;
}

// Or two byte masks
extern inline ruby_mem_access_byte_mask_t or_byte_masks(ruby_mem_access_byte_mask_t mask1, ruby_mem_access_byte_mask_t mask2) {
   return mask1 | mask2;
}

extern inline int get_byte_mask_count(ruby_mem_access_byte_mask_t mask1) {
   return mask1.count();
}
