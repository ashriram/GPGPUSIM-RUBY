...
{ 
  ...
  case TopologyType_CROSSBAR:
    ...
    break;
  case TopologyType_GPGPUSIM_CROSSBAR:
    makeGPGPUSimCrossbar();
    break;
...
// hierarchical switch topology
void Topology::makeGPGPUSimCrossbar()
{
  ...
  for (int i=0; i<RubyConfig::numberOfL1CachePerChip(); i++) {
    if( (i % g_PROCS_PER_CLUSTER) == 0) {
      SwitchID new_switch_cluster = newSwitchID();  // internal switch id #
      last_level.insertAtBottom(new_switch_cluster);
    }

    SwitchID new_switch = newSwitchID();
    addLink(MachineType_base_number(MachineType_L1Cache)+i, new_switch, NETWORK_LINK_LATENCY);
    addLink(new_switch, last_level[last_level.size()-1], NETWORK_LINK_LATENCY);
  }
  ...
  for (int i=0; i<RubyConfig::numberOfDirectoryPerChip(); i++) {
    SwitchID new_switch = newSwitchID();  // internal switch id #
    addLink(MachineType_base_number(MachineType_Directory)+i, new_switch, NETWORK_LINK_LATENCY);
    last_level.insertAtBottom(new_switch);
  }

  SwitchID root_switch = newSwitchID();
  for (int i=0; i<last_level.size(); i++) {
    addLink(last_level[i], root_switch, NETWORK_LINK_LATENCY);
  }
  ...
  for (int i=0; i<out_level.size(); i++) {
     addLink(root_switch, out_level[i], NETWORK_LINK_LATENCY);
  }
}
