...
NetworkInterface_d::NetworkInterface_d(int id, int virtual_networks, GarnetNetwork_d *network_ptr)
{
	...
	m_flit_destination.setSize((int) MachineType_NUM);
	for(int i=0; i<(int) MachineType_NUM; i++)
	   m_flit_destination[i] = 0;
}
...
bool NetworkInterface_d::flitisizeMessage(MsgPtr msg_ptr, int vnet)
{
	...
	for(int ctr = 0; ctr < dest_nodes.size(); ctr++) // loop because we will be converting all multicast messages into unicast messages
	{
      ...
      m_flit_destination[m] += num_flits;
	}
	return true ;
}
...
/*********** Picking messages destined for this NI **********/
	...
	{
	   flit_d* t_flit = inNetLink->peekLink();
	   // Check if buffer space is available downstream
	   //if( (t_flit->get_type() == TAIL_ || t_flit->get_type() == HEAD_TAIL_)==false || outNode_ptr[t_flit->get_vnet()]->areNSlotsAvailable(1) ) {
	      t_flit = inNetLink->consumeLink();
         ...
         delete t_flit;
	   //} else {
	      //printf("cycle=%d DOWNSTREAM BUSY\n", (int)g_eventQueue_ptr->getTime());
	      //g_eventQueue_ptr->scheduleEvent(this, 1);
	      //g_eventQueue_ptr->scheduleEvent(outNode_ptr[t_flit->get_vnet()]->getConsumer(), 1);
	      //g_eventQueue_ptr->scheduleEvent(inNetLink, 1);
	   //}
	}
...
Vector<long long> NetworkInterface_d::getFlitDestinationStats() {
   return m_flit_destination;
}
