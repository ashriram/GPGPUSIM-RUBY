...
{	double average_link_utilization = 0;
   long long total_flits_on_link = 0;
   long long L1_L2_flits_transferred = 0;
	Vector<double > average_vc_load;
	Vector<long long> link_flits_transferred;
	average_vc_load.setSize(m_virtual_networks*NetworkConfig::getVCsPerClass());
	link_flits_transferred.setSize(m_link_ptr_vector.size());
	...
	for(int i = 0; i < m_link_ptr_vector.size(); i++) 
	{
		...
		for(int j = 0; j < vc_load.size(); j++)
		{
			...
			link_flits_transferred[i] += vc_load[j];
			total_flits_on_link += vc_load[j];
		}
	}
	// Let's get the flits generated at L1 and L2 by counting the outbound flits at L1 and L2
	// Can't use L2 here because L2 connects to directory as well
	long long L1_out_flits = 0;
	long long L2_out_flits = 0;
	for(int i=0; i<RubyConfig::numberOfL1CachePerChip(); i++) {
	   // output from L1
   ...
   L1_L2_flits_transferred += L1_out_flits + L2_out_flits;
	...
	out << "Network interface outbound traffic" << endl;
	for(int i=0; i<m_ni_ptr_vector.size(); i++) {
	   // Get interface machineType
      int m;
      for(m = 0; m < (int) MachineType_NUM; m++)
         if((i >= MachineType_base_number((MachineType) m)) && i < MachineType_base_number((MachineType) (m+1)))
            break;

	   Vector<long long> flit_destinations = m_ni_ptr_vector[i]->getFlitDestinationStats();
	   out << "NI_" << i << " (" << MachineType_to_string((MachineType)m) << ") : ";
	   for(int m=0; m<(int) MachineType_NUM; m++) {
	      out << "[" << MachineType_to_string((MachineType)m) << "]=" << flit_destinations[m] << " ";
	   }
	   out << endl;
	}

   out << "L1_L2_flits_transferred = " << L1_L2_flits_transferred << endl;
   out << "L1_out_flits = " << L1_out_flits << endl;
   out << "L2_out_flits = " << L2_out_flits << endl;
   out << "-------------" << endl;
   /*
   for(int i=0; i<m_link_ptr_vector.size(); i++) {
      out << "Flits per link[" << i << "] = " << link_flits_transferred[i] << endl;
   }
   out << "-------------" << endl;
   */
   ...
   out << "total_flits_on_link = " << total_flits_on_link << endl;
	out << "-------------" << endl;
   for(int i = 0; i < NetworkConfig::getVCsPerClass()*NUMBER_OF_VIRTUAL_NETWORKS; i++)
   {
      out << "Total VC Load [" << i << "] = " << ((long)average_vc_load[i]) << " flits" << endl;
   }
   out << "-------------" << endl;
	for(int i = 0; i < NetworkConfig::getVCsPerClass()*NUMBER_OF_VIRTUAL_NETWORKS; i++)
	{
	...
	for(int i = 0; i < m_link_ptr_vector.size(); i++)
	{
	    double static_link_power = 0.0;
	    double dynamic_link_power = 0.0;
	    //m_link_ptr_vector[i]->printStats(out);
		m_total_link_power += m_link_ptr_vector[i]->calculate_power(&static_link_power, &dynamic_link_power);
		m_static_link_power += static_link_power;
		m_dynamic_link_power += dynamic_link_power;
    ...
    out << "Dynamic_Link_Power = " << m_dynamic_link_power << endl;
    out << "Static_Link_Power = " << m_static_link_power << endl;
	out << "Total_Link_Power = " << m_total_link_power << endl;
	out << "Total Link Area = " << m_total_link_area << " uM^2 " << endl;
    //out << "Total Router Power = " << m_total_router_power << <<endl;
	double m_total_router_power = 0.0;
	double m_dynamic_router_power = 0.0;
	double m_static_router_power = 0.0;
    m_total_router_powerarea.PrintPower(out, &m_total_router_power, &m_static_router_power, &m_dynamic_router_power);
    out << "Dynamic_Router_Power = " << m_dynamic_router_power << endl;
    out << "Static_Router_Power = " << m_static_router_power << endl;
    out << "Total_Router_Power = " << m_total_router_power << endl;
    m_total_router_powerarea.PrintArea(out);

    out << endl;
    out << "Total_Dynamic_Power = " << m_dynamic_link_power+m_dynamic_router_power << endl;
    out << "Total_Static_Power = " << m_static_link_power+m_static_router_power << endl;
    out << "Total_Power = " << m_total_link_power+m_total_router_power << endl;
	out << "-------------" << endl;
}
