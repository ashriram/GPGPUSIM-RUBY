...
public:
  ...
  void writeCallback(const Address& address, uint64 memfetch);
  void readCallback(const Address& address, uint64 memfetch);
  void readCallback(const Address& address, MemfetchSet mfset);
  ...
  void memfenceCallback(int thread);
  ...
  void makeMemfenceRequest(const CacheMsg& request);
  void makeIdealInvRequest(const CacheMsg& request);
  bool doRequest(const CacheMsg& request);
  void issueRequest(const CacheMsg& request);
  void issueSkipL1Request(const CacheMsg& request);
  ...
  void removeSkipL1Request(const CacheMsg& request);
private:
  // Private Methods
  ...
  void insertSkipL1Request(const CacheMsg& request);

  void checkOutstandingRequests() const;
  ...
  // Skipped L1 requests
  Map<uint64, CacheMsg>* m_skipL1RequestTable_ptr;
