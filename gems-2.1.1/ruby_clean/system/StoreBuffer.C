...
void StoreBuffer::insertStore(const CacheMsg& request) 
{
  ...
  if(type == CacheRequestType_ST) {
    g_system_ptr->getDriver()->hitCallback(m_chip_ptr->getID(), subblock, type, request.getSpace(), threadID, request.getmemfetch());
...
void StoreBuffer::callBack(const Address& addr, DataBlock& data)
{
  ...
  if(type == CacheRequestType_ST) {
    ...
    g_system_ptr->getDriver()->hitCallback(m_chip_ptr->getID(), peek().m_subblock, type, MemorySpaceType_NULL, threadID, 0);
    m_seen_atomic = false;
...
void StoreBuffer::processHeadOfQueue()
{
  if(!isEmpty() && !m_pending) {
    ...
    CacheMsg request(entry.m_subblock.getAddress(), entry.m_subblock.getAddress(), entry.m_type, entry.m_pc, entry.m_access_mode, entry.m_size, PrefetchBit_No, 0, Address(0), entry.m_thread, 0, false,MemorySpaceType_NULL,false,0,0);
