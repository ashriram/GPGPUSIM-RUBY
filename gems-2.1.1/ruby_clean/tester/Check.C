...
void Check::initiatePrefetch(Sequencer* targetSequencer_ptr)
{
  ...
  CacheMsg request(m_address, m_address, type, m_pc, m_access_mode, 0, PrefetchBit_Yes, 0, Address(0), 0 /* only 1 SMT thread */, 0, false, MemorySpaceType_NULL, false, 0,0);
...
void Check::initiateAction()
{
  ...
  CacheMsg request(Address(m_address.getAddress()+m_store_count), Address(m_address.getAddress()+m_store_count), type, m_pc, m_access_mode, 1, PrefetchBit_No, 0, Address(0), 0 /* only 1 SMT thread */, 0, false,MemorySpaceType_NULL,false, 0,0);
...
void Check::initiateCheck()
{
  ...
  CacheMsg request(m_address, m_address, type, m_pc, m_access_mode, CHECK_SIZE, PrefetchBit_No, 0, Address(0), 0 /* only 1 SMT thread */, 0, false, MemorySpaceType_NULL, false, 0,0);
  Sequencer* sequencer_ptr = initiatingSequencer();
  if (sequencer_ptr->isReady(request) == false) {
