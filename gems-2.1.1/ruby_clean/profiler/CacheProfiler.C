  ...
  : m_requestSize(-1)
{
  ...
  m_missTypeVec_ptr = new Vector<int>;
  m_missTypeVec_ptr->setSize(int(GenericRequestType_NUM));
...
void CacheProfiler::printStats(ostream& out) const
{
  ...
  out << description << "_total_requests: " << m_requests << endl;
  out << description << "_total_misses: " << m_misses;
  if(m_requests > 0)
     out << "\t(" << (100.0*((double)m_misses / (double)m_requests)) << "%)";
  out << endl;
  ...
  // Print misses breakdown
  int misses = 0;
  for(int i=0; i<int(GenericRequestType_NUM); i++) {
    misses += m_missTypeVec_ptr->ref(i);
  }
  assert(m_misses == misses);
  if (misses > 0) {
    for(int i=0; i<int(GenericRequestType_NUM); i++){
       if (m_missTypeVec_ptr->ref(i) > 0) {
         out << description << "_miss_type_" << GenericRequestType_to_string(GenericRequestType(i)) << ":   "
            << m_missTypeVec_ptr->ref(i) << "\t"
            << "(" << (100.0 * double((m_missTypeVec_ptr->ref(i)))) / double(misses) << ")"
            << "%" << endl;
       }
    }
    out << endl;
    ...
    for(int i=0; i<AccessModeType_NUM; i++){
      if (m_accessModeTypeHistogram[i] > 0) {
        out << description << "_access_mode_type_" << (AccessModeType) i << ":   " << m_accessModeTypeHistogram[i] 
            << "    " << (100.0 * m_accessModeTypeHistogram[i]) / misses << "%" << endl;
      }
    }
    */
  }
...
void CacheProfiler::addRequestSample(GenericRequestType requestType, int msgSize, bool miss)
{
  m_requests++;
  ...
  if(miss) {
     m_misses++;
     m_missTypeVec_ptr->ref(requestType)++;
     m_demand_misses++;
  }
