...
public:
  ...
  void profilePrivateL1Eviction();  // profile private L1 evictions
...
private:
  ...
  integer_t m_numtbeQuery;
  integer_t m_busytbeCount;
  ...
  // For profiling bandwidth requests
  Map<string,unsigned long>* m_profiling_bandwidth;
  Map<string,unsigned long>* m_profiling_bandwidth_alt;

  // For profiling cache block lifetimes
  Histogram m_blockLifetimes_pow2;                          // pow2 histogram of all lifetimes
  Histogram m_blockLifetimes_WRITE_pow2;                    // pow2 histogram of WRITE lifetimes
  Histogram m_blockLifetimes_EVICT_pow2;                    // pow2 histogram of EVICT lifetimes
  Map<unsigned,Histogram>* m_blockLifetimes_perPC_pow2;     // pow2 histogram of lifetimes for each unique PC
  Map<unsigned,Histogram>* m_blockLifetimes_perAddr_pow2;   // pow2 histogram of lifetimes for each address
  int m_blockLifetimes_perAddr_granularity;                 // granularity in bytes in which to track addresses

  // For profiling CL stalls
  Histogram m_cl_stall_evict;
  Histogram m_cl_stall_fence;
